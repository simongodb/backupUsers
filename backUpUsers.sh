...
# COMMENT BEFORE RUNNING CODE
# THE SCRIPT WILL BEHAVE AS DESCRIBED BELOW:
# 1- THE SCRIP WILL TAKE AN ARGUMENT (A TEXT FILE)
# WHICH IS SUPPOSED TO HAVE A LIST OF USER NAMES
# 2- WE WILL LOOP THROUG THE LIST ADN VERIFY WHETHER:
# - THE USER EXISTS
# - IF .BACKUP EXISTS
# IF NOT WE WON'T PERFORM ANY BACKUPS
# 2- IF CONDITIONS ARE MET WE WILL: 
# CHECK IF BACKUP.TAR EXIST IN /VAR AND EXTRACT IT TO /TMP/BACKUP
# LOOP THROUGH FILE IN HOME DIRECTORY AND SEE IF: 
# - FILES IN HOME VS TMP/BACKUP/USER ARE SIMILAR
# IF YES, CHANGE FILE NAME AND COPY LATEST VERSION OF FILE FROM HOME TO TMP
# IF NO, COPY LATEST VERISON OF FILES FROM HOME TO TMP...
# 3- COMPRESS USER BACKUP AND MV TO ?VAR
